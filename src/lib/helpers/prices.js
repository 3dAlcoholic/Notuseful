...
import { FARM_TOKEN_ADDRESS, NATIVE_TOKEN_ADDRESS, STABLECOIN_ADDRESS, PULSEX_V2_FACTORY_ADDRESS, PULSEX_V2_ROUTER_ADDRESS } from '$lib/config';
...
export const toUsd = async(amount) => {
    try {
        let reserves = await Factory.getReservesFromTokens(PULSEX_V2_FACTORY_ADDRESS, NATIVE_TOKEN_ADDRESS, STABLECOIN_ADDRESS);
        let quote = await Router.quote(PULSEX_V2_ROUTER_ADDRESS, amount, reserves[0], reserves[1]);
